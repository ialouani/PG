...
static void free__SET(){
  ...
  set__add(s1,4);
  set__free(s1);
  //assert(set__is_empty(s1)==1);(struct set* premier)
  //ca veut pas dire free(struct set*)=>cet ensemble pointe vers
  //un ensemble d'éléments vide.
...
int main(){
  ...
  adding_elmt__set();
  removing_elmt__set();
  free__SET();test_set__filter2();test_set__filter();
